...
class SceneManager {
  ...
  _getOrCreateObject(id, type, layer) {
    ...
    const obj = { id, type, layer: layer || "ui", element: el, data: {}, hidden: false };
  ...
  _applyGeometry(el, geometry) {
    ...
    if (geometry.anchor) {
      el.style.transform = `translate(${geometry.anchor === "center" ? "-50%,-50%" : geometry.anchor === "left" ? "0,-50%" : geometry.anchor === "right" ? "-100%,-50%" : "0,0"})`;
    }
    if (geometry.rotation != null) el.style.transform = `rotate(${geometry.rotation}deg)`;
  ...
  _objectHide(cmd) {
    ...
    if (cmd.transition) this.transitionEngine.apply(obj.element, { type: "fadeOut", ...cmd.transition });
  }
  ...
  _shapeRect(cmd) {
    ...
    const svg = `<svg width="${w}" height="${h}" style="overflow:visible;">
      <rect x="${geom.x || 0}" y="${geom.y || 0}" width="${w}" height="${h}"
  ...
  _youtubeSegment(cmd) {
    ...
    const iframe = document.createElement("iframe");
    iframe.src = `https://www.youtube.com/embed/${videoId}?start=${start}&end=${end}&autoplay=1&rel=0`;
    iframe.allow = "accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture";
    iframe.allowFullscreen = true;
    iframe.style.width = "100%";
    iframe.style.height = "100%";
...
function createBlackboardSDK(agentId, options = {}) {
  const sdk = {
    ...
    table(id, rows, options = {}) {
      return sdk.command("table.show", { id, content: { headers: options.headers || [], rows }, layer: options.layer || "ui", geometry: options.geometry, style: options.style, transition: options.transition });
    },
