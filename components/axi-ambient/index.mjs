...
class TargetSampler {
  ...
  _linePoints(points, x1, y1, x2, y2, count) {
  ...
  _circlePoints(points, cx, cy, radius, count) {
    const steps = Math.max(18, Math.round(count));
    for (let i = 0; i < steps; i += 1) {
  ...
  _arcPoints(points, cx, cy, radius, from, to, count) {
    const steps = Math.max(10, Math.round(count));
    for (let i = 0; i < steps; i += 1) {
      const t = i / (steps - 1);
  ...
  _polygonPoints(points, vertices, countPerEdge) {
    for (let i = 0; i < vertices.length; i += 1) {
      const current = vertices[i];
      const next = vertices[(i + 1) % vertices.length];
      this._linePoints(points, current.x, current.y, next.x, next.y, countPerEdge);
    }
  }

  _rectPoints(points, x, y, width, height, countPerEdge, radius = 0) {
    if (radius <= 0) {
      this._linePoints(points, x, y, x + width, y, countPerEdge);
      this._linePoints(points, x + width, y, x + width, y + height, countPerEdge);
      this._linePoints(points, x + width, y + height, x, y + height, countPerEdge);
      this._linePoints(points, x, y + height, x, y, countPerEdge);
      return;
    }
    const r = Math.min(radius, width * 0.5, height * 0.5);
    this._linePoints(points, x + r, y, x + width - r, y, countPerEdge * 0.6);
    this._arcPoints(points, x + width - r, y + r, r, -Math.PI / 2, 0, countPerEdge * 0.4);
    this._linePoints(points, x + width, y + r, x + width, y + height - r, countPerEdge * 0.6);
    this._arcPoints(points, x + width - r, y + height - r, r, 0, Math.PI / 2, countPerEdge * 0.4);
    this._linePoints(points, x + width - r, y + height, x + r, y + height, countPerEdge * 0.6);
    this._arcPoints(points, x + r, y + height - r, r, Math.PI / 2, Math.PI, countPerEdge * 0.4);
    this._linePoints(points, x, y + height - r, x, y + r, countPerEdge * 0.6);
    this._arcPoints(points, x + r, y + r, r, Math.PI, Math.PI * 1.5, countPerEdge * 0.4);
  }
  ...
  fromAscii(ascii, width, height, targetConfig, wantedCount) {
    ...
    return this._remember(key, points);
  }
  ...
  fromShape(shape, width, height, wantedCount) {
    ...
    const raw = [];
    const edges = Math.max(12, Math.round(count / 14));
    if (name === "spiral") {
      ...
      for (let i = 0; i < count; i += 1) {
        ...
        raw.push({ x: lerp(-96, 96, t), y: Math.sin(t * Math.PI * 8) * 36 });
      }
    } else if (name === "grid") {
      const gx = Math.max(6, Math.round(Math.sqrt(count * Math.max(1, width / Math.max(1, height)))));
      const gy = Math.max(4, Math.ceil(count / gx));
      for (let iy = 0; iy < gy; iy += 1) {
        for (let ix = 0; ix < gx; ix += 1) {
          raw.push({
            x: lerp(-96, 96, gx === 1 ? 0.5 : ix / (gx - 1)),
            y: lerp(-70, 70, gy === 1 ? 0.5 : iy / (gy - 1)),
          });
        }
      }
    } else if (name === "heart") {
      ...
      for (let i = 0; i < 10; i += 1) {
        ...
        const r = i % 2 === 0 ? 100 : 44;
        vertices.push({ x: Math.cos(a) * r, y: Math.sin(a) * r });
      }
      this._polygonPoints(raw, vertices, edges);
      ...
      for (let i = 0; i < count; i += 1) {
        ...
        raw.push({
          x: (95 * Math.cos(t)) / d,
          y: (60 * Math.sin(t) * Math.cos(t)) / d,
        });
      }
    } else if (name === "speech") {
      this._rectPoints(raw, -92, -58, 184, 116, edges, 18);
      this._polygonPoints(raw, [{ x: -18, y: 58 }, { x: 8, y: 58 }, { x: -30, y: 92 }], edges * 0.9);
    } else if (name === "rocket") {
      this._polygonPoints(raw, [{ x: 0, y: -112 }, { x: 54, y: -20 }, { x: 42, y: 96 }, { x: -42, y: 96 }, { x: -54, y: -20 }], edges);
      this._polygonPoints(raw, [{ x: -42, y: 44 }, { x: -88, y: 94 }, { x: -34, y: 80 }], edges * 0.7);
      this._polygonPoints(raw, [{ x: 42, y: 44 }, { x: 88, y: 94 }, { x: 34, y: 80 }], edges * 0.7);
      this._circlePoints(raw, 0, -12, 22, edges * 1.1);
    } else if (name === "trophy") {
      this._polygonPoints(raw, [{ x: -66, y: -68 }, { x: 66, y: -68 }, { x: 42, y: 10 }, { x: -42, y: 10 }], edges);
      this._linePoints(raw, -18, 10, -18, 72, edges * 0.7);
      this._linePoints(raw, 18, 10, 18, 72, edges * 0.7);
      this._rectPoints(raw, -54, 72, 108, 26, edges * 0.7, 6);
      this._arcPoints(raw, -66, -30, 38, Math.PI * 0.5, Math.PI * 1.6, edges);
      this._arcPoints(raw, 66, -30, 38, -Math.PI * 0.6, Math.PI * 0.5, edges);
    } else if (name === "smile") {
      this._circlePoints(raw, 0, 0, 100, edges * 2);
      this._circlePoints(raw, -34, -26, 10, edges * 0.6);
      this._circlePoints(raw, 34, -26, 10, edges * 0.6);
      this._arcPoints(raw, 0, 10, 46, 0.2, Math.PI - 0.2, edges * 1.2);
    } else if (name === "arrow-up") {
      this._polygonPoints(raw, [{ x: 0, y: -116 }, { x: 90, y: -16 }, { x: 32, y: -16 }, { x: 32, y: 106 }, { x: -32, y: 106 }, { x: -32, y: -16 }, { x: -90, y: -16 }], edges);
    } else if (name === "network") {
      const nodes = [
        { x: -82, y: -42 }, { x: 0, y: -88 }, { x: 88, y: -22 }, { x: -52, y: 62 }, { x: 34, y: 88 }, { x: 102, y: 52 },
      ];
      const links = [[0, 1], [1, 2], [0, 3], [1, 4], [2, 5], [3, 4], [4, 5], [0, 4], [1, 3]];
      for (const [from, to] of links) this._linePoints(raw, nodes[from].x, nodes[from].y, nodes[to].x, nodes[to].y, edges * 0.55);
      for (const node of nodes) this._circlePoints(raw, node.x, node.y, 10, edges * 0.5);
    } else if (name === "community") {
      this._circlePoints(raw, -54, -18, 28, edges);
      this._circlePoints(raw, 54, -18, 28, edges);
      this._circlePoints(raw, 0, -44, 34, edges);
      this._arcPoints(raw, 0, 58, 92, Math.PI * 1.05, Math.PI * 1.95, edges * 1.5);
      this._arcPoints(raw, -54, 58, 48, Math.PI * 1.1, Math.PI * 1.9, edges);
      this._arcPoints(raw, 54, 58, 48, Math.PI * 1.1, Math.PI * 1.9, edges);
    } else {
      this._circlePoints(raw, 0, 0, 100, count);
    }
    const fitted = this._fitPoints(raw, width, height, 0.12);
    return this._remember(key, this._downsample(fitted, wantedCount));
  }
...
class Renderer2D {
  _drawShape(ctx, shape, x, y, size) {
    const radius = Math.max(0.35, size);
    if (shape === "square") {
    ...
    if (shape === "plus") {
      ...
      ctx.rect(x - arm, y - radius * 1.3, arm * 2, radius * 2.6);
      ctx.rect(x - radius * 1.3, y - arm, radius * 2.6, arm * 2);
      return;
    }
    if (shape === "star") {
      for (let i = 0; i < 10; i += 1) {
        const a = -Math.PI / 2 + (i / 10) * Math.PI * 2;
        const r = i % 2 === 0 ? radius * 1.45 : radius * 0.65;
  ...
  draw(ctx, engine, alphaField, phase, width, height, intensity, config) {
    ...
    const renderShape = config?.particle?.renderShape || "circle";
...
class AxiAmbientElement extends HTMLElement {
  ...
  _applyDeclarativeAttributes() {
    ...
    if (this.hasAttribute("density")) this._config.particle.count = parseNumberAttribute(this.getAttribute("density"), 800, 100, 8000);
    if (this.hasAttribute("speed")) this._config.speed = parseNumberAttribute(this.getAttribute("speed"), 0.9, 0.1, 4.5);
